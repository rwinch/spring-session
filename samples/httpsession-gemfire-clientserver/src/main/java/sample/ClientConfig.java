...
public class ClientConfig {
	...
	PoolFactoryBean gemfirePool() { // <3>
		...
		poolFactory.setServerEndpoints(Collections.singletonList(new ConnectionEndpoint(
			ServerConfig.SERVER_HOSTNAME, ServerConfig.SERVER_PORT)));
		...
		return new BeanPostProcessor() {
			public Object postProcessBeforeInitialization(
				...
				if (bean instanceof PoolFactoryBean || bean instanceof Pool) {
					Assert.isTrue(waitForCacheServerToStart(ServerConfig.SERVER_HOSTNAME, ServerConfig.SERVER_PORT),
						"GemFire Server failed to start");
				}
